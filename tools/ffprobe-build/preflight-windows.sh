...
for package in mingw-w64-ucrt-x86_64-gcc mingw-w64-ucrt-x86_64-binutils nasm; do
...
cat > "$EVIDENCE/msys2-toolchain-preflight.json" <<EOF
{
  ...
  "package_archive_sha256": {"${package_archives[0]%%=*}": "${package_archives[0]#*=}", "${package_archives[1]%%=*}": "${package_archives[1]#*=}", "${package_archives[2]%%=*}": "${package_archives[2]#*=}"},
  "binding": "PASS"
}
EOF
printf 'status=PASS\ncompiler=%s\nlinker=%s\nassembler=%s\n' "$compiler_path" "$linker_path" "$assembler_path" >> "$STATUS"

...
OUT_INPUT="${PIXEL_ORACLE_OUT:-$ROOT/artifacts/ffmpeg-pixel-oracle-v2/windows}"
case "$OUT_INPUT" in
  /*|[A-Za-z]:/*) OUT="$OUT_INPUT" ;;
  *) OUT="$ROOT/$OUT_INPUT" ;;
esac
PROFILE="$ROOT/tools/ffmpeg-render-pixel-oracle-v2/CODE_F_ROTATION_PIXEL_ORACLE_BUILD_PROFILE_V2.json"
SOURCE_TARBALL="$RUNNER_TEMP/ffmpeg-9.0.1.tar.xz"
SOURCE_DIR="$RUNNER_TEMP/ffmpeg-9.0.1-pixel-oracle-v2-build"
...
SMOKE_FIXTURE="${PIXEL_ORACLE_SMOKE_FIXTURE:-}"
SMOKE_FIXTURE_SHA256="${PIXEL_ORACLE_SMOKE_FIXTURE_SHA256:-}"
SMOKE_WIDTH="${PIXEL_ORACLE_SMOKE_WIDTH:-}"
SMOKE_HEIGHT="${PIXEL_ORACLE_SMOKE_HEIGHT:-}"
...
test -f "$PROFILE"
test -n "$SMOKE_FIXTURE"
test -f "$SMOKE_FIXTURE"
[[ "$SMOKE_FIXTURE_SHA256" =~ ^[0-9a-fA-F]{64}$ ]]
test "$(sha256sum "$SMOKE_FIXTURE" | awk '{print tolower($1)}')" = "$(printf '%s' "$SMOKE_FIXTURE_SHA256" | tr '[:upper:]' '[:lower:]')"
test -n "$SMOKE_WIDTH"
test -n "$SMOKE_HEIGHT"
...
mapfile -t CONFIGURE_ARGS < <(node "$ROOT/tools/ffmpeg-render-pixel-oracle-v2/resolve-profile.mjs")
CONFIGURE_JSON="$(node "$ROOT/tools/ffmpeg-render-pixel-oracle-v2/resolve-profile.mjs" --json)"
...
TOOL_ID="code-f-rotation-pixel-oracle-v2-windows-x86_64-${GITHUB_RUN_ID:-local}"
...
const value = {
  schema_version: '1',
  record_kind: 'CODE_F_ROTATION_PIXEL_ORACLE_V2',
  tool_id: toolId,
  tool_role: 'TEST_ONLY',
  ...
  build_profile: {
    path: 'tools/ffmpeg-render-pixel-oracle-v2/CODE_F_ROTATION_PIXEL_ORACLE_BUILD_PROFILE_V2.json',
    sha256: profileSha,
  },
  ...
  capabilities: {
    ...
    output: ['format_filter', 'scale_filter', 'rgb24_pixel_conversion', 'rawvideo_encoder', 'rawvideo_muxer'],
    network: 'DISABLED',
  },
  invocation_contract: {
...
PIXEL_ORACLE_SMOKE_FIXTURE_SHA256="$SMOKE_FIXTURE_SHA256" PIXEL_ORACLE_SMOKE_WIDTH="$SMOKE_WIDTH" PIXEL_ORACLE_SMOKE_HEIGHT="$SMOKE_HEIGHT" \
  node "$ROOT/tools/ffmpeg-render-pixel-oracle-v2/verify-artifact-v2.mjs" --root "$OUT" --fixture "$SMOKE_FIXTURE"
...
cp "$OUT/evidence/static-capability-inspection-v2.json" "$OUT/transport-root/evidence/static-capability-inspection-v2.json"
cp "$OUT/evidence/rgb24-smoke.stderr.txt" "$OUT/transport-root/evidence/rgb24-smoke.stderr.txt"
tar --format=posix -cf "$OUT/code-f-rotation-pixel-oracle-v2-windows-x86_64.tar" -C "$OUT/transport-root" .
sha256sum "$OUT/code-f-rotation-pixel-oracle-v2-windows-x86_64.tar" > "$OUT/code-f-rotation-pixel-oracle-v2-windows-x86_64.tar.sha256"
echo 'CODE_F_ROTATION_PIXEL_ORACLE_V2_BUILD: PASS'

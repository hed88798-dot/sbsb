...
export function registerIpc(options: {
  ...
  handle(IPC_CHANNELS.productsDelete, (input) => {
    ...
    if (!options.products.delete(request.product_id)) throw new Error('PRODUCT_NOT_FOUND');
    return null;
  });
  ...
  handle(IPC_CHANNELS.copywritingGenerate, (input) =>
    options.copywriting.enqueue(copywritingGenerateRequestV1Schema.parse(input)),
  );

import { copywritingGenerateRequestV1Schema, textGatewayRequestV1Schema, } from '@app/contracts';
...
export class CopywritingService {
    ...
    async #run(input) {
        try {
            if (this.#jobs.require(input.jobId).state !== 'QUEUED')
                return;
            this.#jobs.start(input.jobId);
            const textResult = await this.#client.generate(textGatewayRequestV1Schema.parse({
                schema_version: '1.0',
            ...
            if (this.#jobs.require(input.jobId).state === 'CANCELLED')
                return;
            ...
            if (error instanceof GatewayClientError && error.code === 'CANCELLED') {
                ...
                const message = error instanceof GatewayClientError ? error.message : '文案任务执行失败，请检查后重试';
